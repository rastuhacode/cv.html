// All credits to https://github.com/ts-zen/trycatch
...
/**
 * Wraps an asynchronous task or chains a promise with then/catch, returning a
 * promise that resolves with the resulting {@link TryCatch} tuple.
 *
 * @param task - An async task or a promise.
...
export class ThrownError extends Error {
  ...
  public constructor(cause: unknown) {
    ...
    this.cause = cause
    // Capture the stack trace (V8/Node extension, not in standard lib types).
    const ErrorWithCapture = Error as ErrorConstructorWithCapture
    if (ErrorWithCapture.captureStackTrace) {
      ErrorWithCapture.captureStackTrace(this, ThrownError)
    }
  }
}
